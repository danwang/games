import { type Card, type GemColor, type Noble, type TokenColor } from '../model/types.js';
...
import { gemOrder } from './game-ui.js';
...
const cardSurfaceStyles: Readonly<Record<TokenColor, string>> = {
  white: 'from-white via-slate-100 to-slate-400 text-stone-900 border-slate-300/90',
  ...
  black: 'from-slate-700 via-slate-800 to-stone-950 text-stone-50 border-slate-500/80',
};
...
export const getNobleImageSrc = (nobleId: string): string => nobleImageById[nobleId] ?? noble01Image;
...
const GemIcon = ({ color, size }: { readonly color: TokenColor; readonly size: GemPipProps['size'] }) => (
  <span
    className={`relative inline-flex shrink-0 items-center justify-center ${
      size === 'xs' ? 'h-4 w-4' : size === 'sm' || size === 'summary' ? 'h-5 w-5' : 'h-6 w-6'
    }`}
  >
    ...
    return (
      <span
        className={`inline-flex items-center justify-center ${
          size === 'xs' ? 'h-5 w-5' : size === 'sm' || size === 'summary' ? 'h-7 w-7' : 'h-9 w-9'
        }`}
      >
    ...
    >
      ...
      <div
        className={`relative -mx-1.5 -mt-1.5 mb-1 flex items-start justify-between gap-2 rounded-t-[inherit] border-b border-black/8 bg-[linear-gradient(180deg,_rgba(226,232,240,0.56),_rgba(214,211,209,0.28))] px-1.5 pt-1.5 pb-1 ${
          isTiny ? 'min-h-7' : isCompact ? 'min-h-9' : 'min-h-12'
        }`}
      >
        <span
          className={`font-serif leading-none drop-shadow-sm ${
      ...
      <div
        className={`relative ${
          isTiny
            ? 'mt-0.5 grid grid-cols-2 content-end justify-items-start gap-0.5'
            : isCompact
              ? 'mt-1 grid grid-cols-2 content-end justify-items-start gap-1'
...
export const DeckCard = ({
  ...
  return (
    <button
      ...
      className={`relative flex ${
        isCompact ? 'aspect-[5/7] w-full min-w-0 rounded-[1.05rem] p-2' : 'aspect-[5/7] w-full rounded-[1.7rem] p-3'
      } flex-col justify-between overflow-hidden border bg-linear-to-br text-left shadow-lg transition ${
        ...
        {!hideCount ? (
          <span
            className={`font-serif leading-none ${
              isCompact ? 'text-[1.85rem] text-white/72' : 'text-4xl text-white/78'
            }`}
          >
            {remainingCount}
          </span>
